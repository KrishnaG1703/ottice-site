...
- `icon.png` (app icon)
- `otter3d/otter.glb` (3D otter sculpt for the hero) and `otter3d/otter-real.webp` (still fallback when 3D can't render)
...
- The otter sways left/right (about ±30°), bobs slightly, and leans a little toward the cursor wherever it is on the page. There is no hover interaction on the otter itself.
- If WebGL or the model fails to load, show the still render `otter-real.webp` in its place.
